...
const filterData = (categoryName) => {
  ...
  const filtered = [];
  traverseCategory(category, (item) => {
    const nameMatch = !condition.name && !globalSearchText.value || item.name.toLowerCase().includes(condition.name.toLowerCase()) || item.name.toLowerCase().includes(globalSearchText.value.toLowerCase());
